...
use crate::types::{FileAnalysis, FunctionInfo, ClassInfo, ImportInfo, Language, Parameter};
...
impl ASTLayer {
    ...
    pub async fn analyze_file(&self, path: &Path) -> Result<FileAnalysis> {
        ...
        match language {
            ...
            Language::TypeScript | Language::JavaScript => self.parse_typescript(&source, &mut analysis, path),
            _ => {}
        }
    ...
    fn parse_python(&self, source: &str, analysis: &mut FileAnalysis, file: &Path) {
        ...
        for cap in import_re.captures_iter(source) {
            if let Some(module) = cap.get(1) {
                ...
                let names: Vec<String> = names.as_str()
                    .split(',')
